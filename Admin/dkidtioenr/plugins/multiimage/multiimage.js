...
//add20210923
'<div class="file_dig" style="display:none; position: absolute;z-index:99999; width:654px; height:510px; top: 50%; left:50%;margin-top: -255px;margin-left: -327px;background:rgba(0,0,0,0.1);text-align:center; line-height:494px;color:red; font-size:18px;" >上传中...</div>',
...
'<button type="button" onclick="confirm_upload()" class="ke-button-common upload_submit_btn fileSubmit" style="border:1px solid #E4E4E4;border-left:none;border-top:none;border-bottom:2px solid #DDD;border-radius:2px;font-size:12px;color:#232121;padding-left:10px;padding-right:10px;" >确认上传文件</button>',//add ke-button-common
'</form>',
].join('');
...
imageSizeLimit = K.undef(self.imageSizeLimit, '3MB'),//最大上传容量
imageFileTypes = K.undef(self.imageFileTypes, '*.jpg;*.gif;*.png;*.mp4'),//图片格式/类型
...
$(".fileSubmit").css({ "color" : "#000", });

setTimeout(function(){
$('.fileSubmit').text('正在上传中...');
$('.file_dig').css({ "display" : "block",  });
//alert('正在上传中');
}, 1500);
...
setTimeout(function(){
$('.file_dig').css({ "display" : "none", });
"$('.fileSubmit').text('上传成功').css({ 'color' : 'red', })";
$('.file_dig').text('正在上传中！！！');
$('.fileSubmit').text('确认上传文件').css({ 'color' : 'red', });
}, 12000);
