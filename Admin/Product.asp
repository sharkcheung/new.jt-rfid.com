...
dim chkContentEx1,chkContentEx2,chkSlidesImgs,chkSummary,chkSlidesFirst
...
<div id="ListNav">
    <ul>
        <li><a href="javascript:void(0);" onclick="ShowBox('Product.asp?Type=2&ModuleId=<%=Fk_Module_Id%>');">添加</a></li>
        <li><a href="javascript:void(0);" onclick="SetRContent('MainRight','<%=Session("NowPage")%>');return false">刷新</a></li>
    </ul>
</div>
<div id="ListTop">
    <%=Fk_Module_Name%>栏目&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="SearchStr" value="<%=SearchStr%>" type="text" class="Input" id="SearchStr" style="vertical-align:middle"/>&nbsp;<input type="button" class="Button" onclick="SetRContent('MainRight','Product.asp?Type=1&ModuleId=<%=Fk_Module_Id%>&SearchStr='+escape(document.all.SearchStr.value));" name="S" Id="S" value="  查询  " style="vertical-align:middle" />&nbsp;&nbsp;请选择栏目：
...
<div id="ListContent">
    ...
    <table width="100%" bordercolor="#CCCCCC" border="1" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <td align="center" class="ListTdTop">转播微博</td>
...
%>
        <tr>
            <td height="20" align="center"><input type="checkbox" name="ListId" class="Checks" value="<%=Rs("Fk_Product_Id")%>" id="List<%=Rs("Fk_Product_Id")%>" /></td>
            <td class="td1">&nbsp;<%=Rs("Fk_Product_Title")%><%If Rs("Fk_Product_Color")<>"" Then%><span style="color:<%=Rs("Fk_Product_Color")%>">■</span><%End If%><%If Rs("Fk_Product_Url")<>"" Then%>[转向链接]<%End If%></td>
            <td align="center" style="display:none"><%=Rs("Fk_Product_FileName")%></td>
            <td align="left"><%If Rs("Fk_Product_Show")=1 Then%><img src="images/caidan1.png"  style="vertical-align:middle"/><%Else%><img src="images/caidan0.png"  style="vertical-align:middle"/><%End If%><%If Rs("Fk_Product_Pic")<>"" Then%><span style="color:#F00">[图]</span><%End If%><a style="display:none;" href="javascript:void(0);" title="<%=Fk_Product_Template%> ">[模]</a><%If InStr(Fk_Product_Recommend,"推荐")>0 Then%><a href="javascript:void(0);" title="<%=Replace(Fk_Product_Recommend,",","")%> ">[推]</a><%End If%><%If trim(Rs("Fk_Product_Ip")&" ")="1" Then%><a href="javascript:void(0);" title="置顶 "><font color=blue style="font-weight:bolder;">[顶]</font></a><%End If%><%If Fk_Product_Subject<>"" Then%><a href="javascript:void(0);" title="<%=Replace(Fk_Product_Subject,",","")%> ">[专]</a><%End If%></td>
            <td align="center"><%=Rs("Fk_Product_Click")%></td>
            <td height="20" align="center"><%=Rs("Px")%></td>
            <td align="center"><a href="http://share.v.t.qq.com/index.php?c=share&a=index&url=<%=server.URLEncode(zfurl)%>&site=<%=server.URLEncode(zfurl)%>%23via%3Dshare_t_weib
&title=<%=server.URLEncode(Rs("Fk_Product_Title")&"("&Rs("Fk_Product_Keyword")&")")%>&pic=<%
if left(Rs("Fk_Product_Pic"),4)<>"http" then
	...
	response.write Rs("Fk_Product_Pic")
			...
			%>" target="_blank"><img style="cursor:pointer;vertical-align:middle" alt="转发到腾讯微博 " src="Images/weiboicon16.png" ></a>&nbsp;<a href="http://service.weibo.com/share/share.php?url=<%=zfurl%>&appkey=1525536596&title=<%=server.URLEncode(Rs("Fk_Product_Title")&"("&Rs("Fk_Product_Keyword")&")")%>&pic=<%if left(Rs("Fk_Product_Pic"),4)<>"http" then
	response.write "http://"&Request.ServerVariables("Server_name")&Rs("Fk_Product_Pic")
else
	response.write Rs("Fk_Product_Pic")
end if
            ...
            <td align="left"><a title="修改 " href="javascript:void(0);" onclick="ShowBox('Product.asp?Type=4&ModuleId=<%=Fk_Module_Id%>&Id=<%=Rs("Fk_Product_Id")%>');"><img src="images/edit.png" style="vertical-align:middle"></a> <a title="删除 " href="javascript:void(0);" onclick="DelIt('您确认要删除“<%=Rs("Fk_Product_Title")%>”，此操作不可逆！','Product.asp?Type=6&Id=<%=Rs("Fk_Product_Id")%>','MainRight','<%=Session("NowPage")%>');"><img src="images/del.png" style="vertical-align:middle"></a></td>
        </tr>
<%
			Rs.MoveNext
			i=i+1
...
%>
        <tr>
            ...
            <input type="submit" value="删 除" class="Button" onClick="if(confirm('此操作无法恢复！！！请慎重！！！\n\n确定要删除选中的图文吗？')){Sends('DelList','Product.asp?Type=7',0,'',0,1,'MainRight','<%=Session("NowPage")%>');}" style="vertical-align:middle">
...
<span class="colorpicker" onclick="colorpicker('colorpanel_title','set_title_color');" title="标题颜色" style="vertical-align:middle"></span>
                        ...
                        <tbody><tr>
                          <td style="width:260px;border-bottom:0" valign="top">
                            <div class="uploadimgArea" style="margin-left:8px;">
                               <div class="top">
                                 <span class="fr sysBtn" style="margin:-3px 0px 0px 0px;"><a id="MutiImg" class="Button" style="cursor:pointer;display: inline-block;width: 90px;"><b class="icon icon_add"></b>添加图片</a></span>
                                 多图片(最多允许10张)</div>
     ...
     <tr>
		...
		<div class="image_grid">
                              ...
                              <td valign="top" style="padding:0px;border-bottom:0px;">
                                  <div class="imageBox" id="ThumbnailBox">image001.dgcloud01.qebang.cn
    ...
    <tr>
        ...
        <td colspan="2"><textarea name="Fk_Product_Summary"  style="border: 1px solid #D3E3F0;width:300px;padding-top:5px;padding-left:5px;" id="Fk_Product_Summary" rows="5" cols="50" disabled></textarea>
		<div style="width:300px;"><input type="checkbox" id="GetAbstract" name="GetAbstract" value="1" checked="checked" style="vertical-align:middle;"> <label for="GetAbstract" style="vertical-align:middle;">采用产品详细的前<%=SiteMini%>个字作为摘要</label></div></td>
	  </tr>
	  <%end if%>
     <tr>
	  ...
	  <%if chkContentEx1 then%>
                              ...
                              <li><a href="javascript:void(0);" onclick="TabSwitch('J_TabBar','TabBar_',2);return false;"><span><%=getDesc("fk_product","Fk_Product_ContentEx1")%></span></a></li>
                                     
	  <%end if
	  if chkContentEx2 then%>
                              <li><a href="javascript:void(0);" onclick="TabSwitch('J_TabBar','TabBar_',3);return false;"><span><%=getDesc("fk_product","Fk_Product_ContentEx2")%></span></a></li>
...
%>
	...
	Rs.Close
%>
            </select>            &nbsp; &nbsp; 点击量：<input name="Fk_Product_click" type="text" id="Fk_Product_click" class="Input" size="6" maxlength="6" value="<%=rnd_num%>" style="vertical-align:middle;"></td>
    </tr>
</table>
</div>
...
<span class="colorpicker" onclick="colorpicker('colorpanel_title','set_title_color');" title="标题颜色" style="vertical-align:middle"></span>
          ...
          <table border="0" cellpadding="0" cellspacing="0" width="100%">
                        <tbody><tr>
                          <td style="width:260px;border-bottom:0" valign="top">
                            <div class="uploadimgArea" style="margin-left:8px;">
                               <div class="top">
                                 <span class="fr sysBtn" style="margin:-3px 0px 0px 0px;"><a id="MutiImg" style="cursor:pointer;display: inline-block;width:90px;" class="Button"><b class="icon icon_add"></b>添加图片</a></span>
                                 多图片(最多允许10张)</div>
	...
	<tr>
		...
		<div class="image_grid">
                              ...
                              <td valign="top" style="padding:0px;border-bottom:0px;">
                                  <div class="imageBox" id="ThumbnailBox">image001.dgcloud01.qebang.cn
                                  <img class="img" id="SlImg" src="<%if Fk_Product_Pic<>"" then response.write Fk_Product_Pic :else : response.write "http://image001.dgcloud01.qebang.cn/website/ext/images/image.jpg"%>">                                  </div>                              </td>
                              <td valign="top" style="padding:0px 0px 0px 6px;border-bottom:0px;">
    ...
    <tr>
        <td height="28" align="right">内容摘要：</td>
        <td><textarea name="Fk_Product_Summary"  style="border: 1px solid #D3E3F0;width:300px;padding-top:5px;padding-left:5px;"  id="Fk_Product_Summary" rows="5" cols="50" <%if Fk_Product_Summary="" then response.write "disabled"%>><%=Fk_Product_Summary%></textarea>
    ...
    <%end if%>
	  ...
	  <%if chkContentEx1 then%>
                      <div class="product-infoitem" id="TabBar_2" style="display:none;">
						  <textarea name="Fk_Product_ContentEx1" class="<%=Bianjiqi%>" id="Fk_Product_ContentEx1" rows="15" style="width:100%;"><%=Fk_Product_ContentEx1%></textarea>
                      </div>
	  <%end if
	  if chkContentEx2 then%>
                      <div class="product-infoitem" id="TabBar_3" style="display:none;">
						  <textarea name="Fk_Product_ContentEx2" class="<%=Bianjiqi%>" id="Fk_Product_ContentEx2" rows="15" style="width:100%;"><%=Fk_Product_ContentEx2%></textarea>
                      </div>
	  <%end if%>
...
%>
	...
	Rs.Close
%>
            </select> &nbsp; &nbsp; 点击量：<input name="Fk_Product_click" type="text" id="Fk_Product_click" class="Input" size="6" maxlength="6" value="<%=Fk_Product_click%>" style="vertical-align:middle;"></td>
    </tr>
</table>
</div>
<div id="BoxBottom" style="width:96%;">
