...
<div id="ListNav">
    <ul>
        <li><a href="javascript:void(0);" onclick="SetRContent('MainRight','<%=Session("NowPage")%>');return false">刷新订单</a></li>
        <li><a href="javascript:void(0);" onclick="SetRContent('MainRight','zhifu.asp?Type=1');return false">支付设置</a></li>
        <li><a href="javascript:void(0);" onclick="SetRContent('MainRight','songhuo.asp?Type=1');return false">送货设置</a></li>
    </ul>
</div>
<div id="ListTop">
    订单管理　　　　　　　<select name="select" onChange="SetRContent('MainRight','editdingdan.asp?zhuangtai='+this.options[this.selectedIndex].value);" style="font-weight: 700" ><base target=Right> 
...
<tr><td valign="top"> 
	...
	<tr > 
	<td width="21%" align="center" class="ListTdTop">订单号</td>
	<td width="15%" align="center" class="ListTdTop">下单会员</td>
	<td width="24%" align="center" class="ListTdTop">订货人姓名</td>
	<td width="13%" align="center" class="ListTdTop">付款方式</td>
	<td width="15%" align="center" class="ListTdTop">订单状态</td>
	<td width="12%" align="center" class="ListTdTop">操作</td>
	</tr>
<%set Rs=server.CreateObject("adodb.recordset")
   'if namekey="" then	   '//按状态查询
    ...
    <td align="center">
	<input type="button" onclick="ShowBox('viewdingdan.asp?oid=<%=rs("id")%>&dan=<%=rs("order_id")%>&username=<%=trim(rs("u_id"))%>');" class="Button" name="button" id="button" value="详 细" />
    <input style="display:none" type="checkbox" name="selectbookid" value="<%=rs("id")%>"></td>
		...
		Wend%>
	<tr > 
<td style="" height="30" colspan="6" align="right">全选 
<input type="checkbox" name="checkbox" value="Check All" onClick="var checkboxs=document.getElementsByName('orderid');for (var i=0;i<checkboxs.length;i++) {var e=checkboxs[i];e.checked=!e.checked;}">
<input type="button" class="Button" name="Submit" value="删 除" onClick="var str='';$('input[name=orderid]').each(function(){if(this.checked){if(str==''){str=$(this).val();}else{str+=','+$(this).val()}}});DelIt('您确认要删除，此操作不可逆！','editdingdan.asp?Type=4&orderid='+str,'MainRight','<%=Session("NowPage")%>');">
&nbsp;</td>
</tr>
        <tr>
            <td height="30" colspan="6" style="text-align:center;">&nbsp;<%Call FKFun.ShowPageCode("editdingdan.asp?Page=",PageNow,PageAll,PageSizes,PageCounts)%></td>
        </tr>
		<%
	Else
%>
