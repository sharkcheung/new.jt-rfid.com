...
'---缓存扩展：缓存时间、缓存标志
'缓存时间，单位分钟
Const WebCacheTime = 1
'缓存标示，用于一个空间安装多个系统时使用
Const WebCacheFlag = "Cache"
Dim asp : Set asp = New EasyASP
'------------------------
...
Class EasyAsp
	...
	Function Test(ByVal Str, ByVal Pattern)
		...
		Select Case Lcase(Pattern)
			...
			Case "chinese"	Pa = "^[\u0391-\uFFE5]+$"
			Case "username"	Pa = "^[A-Za-z]\w{5,15}$"
			Case "password" Pa = "^[A-Za-z0-9~`!@#$%^&*()_+{}:<>?\-=[\];\,.\/\\、||]{6,16}$"
...
Class EasyAsp_db
	...
	End Function
	Public Function Json(ByVal jRs, ByVal jName,ByVal jArr)
		On Error Resume Next
		Dim tmpStr, rs, fi, i, j, k, o, arrJsn, jsn, isE,tName,tValue : i = 0
		isE = False
		o = Easp_Param(jName)
		If Not Easp_isN(o(1)) Then
			jName = o(0)
		...
		if IsArray(jArr) then
			if ubound(jArr)=0 then
				if instr(jArr(0),":")>0 then
					arrJsn=split(jArr(0),":")
					jsn=""""&arrJsn(0)&""":"&arrJsn(1)&","
				end if
			else
				for k=0 to ubound(jArr)
					if instr(jArr(k),":")>0 then
						arrJsn=split(jArr(k),":")
						jsn=jsn&""""&arrJsn(0)&""":"&arrJsn(1)&","
					end if
				next
			end if
		else
...
'add by shark 2011-11-22

Function chk_session(strName)
	chk_session=false
	if not Easp_isN(session(strName)) then
		chk_session=true
	end if
End Function

function NameToId(StrName,tbName,fdName,fdID)
	dim Irs,Isql
	Isql="select "&fdID&" from "&tbName&" where "&fdName&"='"&StrName&"'"
...
function IdToName(intID,tbName,fdName,fdID)
	dim Irs,Isql
	Isql="select "&fdName&" from "&tbName&" where "&fdID&"="&intID
	set Irs=conn.execute(Isql)
	if not Irs.eof then
		IdToName=Irs(0)
	else
