...
<script src="../caiji2/jquery-1.4.4.js" type="text/javascript"></script>
...
$(document).ready(function()
{
	...
	//alert($(".cj_content").find("#description").html());
   ...
   $("#caiji").click(function()
   {
	  ...
	  type:"POST",
	  url:"sosave.asp",
	  data:"tit="+escape($("#main_s_content h3").html())+"&cont="+escape($("#main_s_content .cj_content").html())+"&Module="+$("#mdltype").val(),
	  async:false,
...
dim t,para
t=0
para=trim(Request.querystring("url"))
if t="0" then


   url=para
url = replace(url,"<font>","")
url = replace(url,"</font>","")
   wstr=getHTTPPage(url,"gb2312")
 '  dim filen,rannum
	'Randomize
	'rannum=int(100*rnd) mod 9
	'filen="temp/"&ToUnixTime(now(),+8)&rannum&".html"
	tit=strCut(wstr,"<div id=""title"">","</div>",2)
	tit=strCut(tit,"<h1>","</h1>",2)
   	wstr=strCut(wstr,"<div id=""artical"">","</div>",2)
	wstr=py_z_replace(wstr,"(<script[\s\S]*?<\/script>)*","")
	wstr=py_z_replace(wstr,"(<p class=""editorN"">[\s\S]*?</p>)*","")
	wstr=py_z_replace(wstr,"(<div id=""pageno"">[\s\S]*?</a>)*","")
   	'WriteToTextFile filen,wstr,"gb2312"
	'html=py_z_replace(html,"(<a[\s\S]*?href=""http://www.*?hc360.com[\s\S]*?</a>)*","")
	wstr=py_z_replace(wstr,"(<p align=""left"">[\s\S]*?<a[\s\S]*?href=.https://itunes\.apple\.com[\s\S]*?</p>)*","")
	wstr=py_z_replace(wstr,"(<a[\s\S]*?</a>)*","")
	wstr=py_z_replace(wstr,"(<p[\s\S]*?><iframe[\s\S]*?</iframe></p>)*","")
	wstr=py_z_replace(wstr,"(<iframe[\s\S]*?</iframe>)*","")
	wstr=py_z_replace(wstr,"(<form[\s\S]*?</form>)*","")
	wstr=py_z_replace(wstr,"(<style[\s\S]*?</style>)*","")
...
response.write "<div id=""main_s_content"" style='padding-top:50px;'><h3 style='font-size:15pt;text-align:center;'>"&tit&"</h3><div class=""cj_content"">"&wstr&"</div></div>"
