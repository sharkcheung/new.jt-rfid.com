...
<div id="ListNav">
    <ul>
        <li><a href="javascript:void(0);" onclick="ShowBox('FormMaker.asp?Type=2');">添加</a></li>
        <li><a href="javascript:void(0);" onclick="SetRContent('MainRight','<%=Session("NowPage")%>');return false">刷新</a></li>
    </ul>
</div>
<div id="ListTop">
   <input name="SearchStr" value="<%=SearchStr%>" type="text" class="Input" id="SearchStr" style="vertical-align:middle;"/>&nbsp;<input type="button" class="Button" onclick="SetRContent('MainRight','FormMaker.asp?Type=1&SearchStr='+escape(document.all.SearchStr.value));" name="S" Id="S" value="  查询  "  style="vertical-align:middle;"/>
</div>
<div id="ListContent">
    ...
    <table width="100%" bordercolor="#CCCCCC" border="1" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <td align="center" class="ListTdTop">表单名称</td>
            <td align="center" class="ListTdTop">是否开始</td>
...
%>
	...
	End if%></td>
            <td align="center"><%If Rs("Ext_Form_Statu")=0 Then%><img src="images/caidan1.png" style="vertical-align:middle;"/><%Else%><img src="images/caidan0.png" style="vertical-align:middle;"/><%End If%></td>
            <td align="center"><a title="字段列表 " href="javascript:void(0);" onclick="SetRContent('MainRight','FormMaker.asp?Type=8&Formid=<%=Rs("Id")%>&tb=<%=Rs("Ext_Table_Name")%>');return false;">字段列表</a>&nbsp;| <a title="HTML调用 " href="javascript:void(0);" onclick="ShowBox('FormMaker.asp?Type=14&Id=<%=Rs("Id")%>&A=HTML');return false;">HTML调用</a>&nbsp;| <a title="修改 " href="javascript:void(0);" onclick="ShowBox('FormMaker.asp?Type=4&Id=<%=Rs("Id")%>');return false;">修改</a>&nbsp;| <a title="删除 " href="javascript:void(0);" onclick="DelIt('确认删除？此操作会将表单相关内容全部删除！','FormMaker.asp?Type=6&Id=<%=Rs("Id")%>','MainRight','<%=Session("NowPage")%>');return false;">删除</a></td>
        </tr>
<%
			Rs.MoveNext
			i=i+1
...
<div id="BoxContents" style="width:700px;">
	<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
            <td>&nbsp;<%If A="HTML" Then %>
    <textarea name="textarea" cols="50" rows="24" class="TextArea" id="textarea" style="width:600px;"><% Call ListForm(modeid) %></textarea>
	<%Else %><textarea name="textarea" cols="10" rows="4" id="textarea" style="width:600px;"><%response.write "<script language=""javascript"" type=""text/javascript"" src=""plus/Form/ACT.F.ASP?ModeID="&ModeID&"""></script>"%></textarea>
	<%End If %></td>
        </tr>
	</table>
</div>
<div id="BoxBottom" style="width:680px;">
        <input id="Button1" type="button" value="复 制" class="Button" onClick="A_CP('textarea')" /> &nbsp; <input type="button" onclick="$('#Boxs').hide();$('select').show();" class="Button" name="button" id="button" value="关 闭" />
</div>
</form>
...
<table width="97%" border="0" align="center" cellpadding="0" cellspacing="0"> 
    <tr>
        <td height="25" align="right">表单状态：</td>
...
<div id="BoxBottom" style="width:96%;">
        <input type="submit" onclick="Sends('FormAdd','FormMaker.asp?Type=3',0,'',0,1,'MainRight','<%=Session("NowPage")%>');" class="Button" name="button" id="button" value="添 加" />
...
<div id="BoxBottom" style="width:96%;">
        ...
        <input type="submit" onclick="Sends('ArticleEdit','FormMaker.asp?Type=5',0,'',0,1,'MainRight','<%=Session("NowPage")%>');" class="Button" name="button" id="button" value="修 改" />
        <input type="button" onclick="$('#Boxs').hide();$('select').show();" class="Button" name="btnClose" id="btnClose" value="关 闭" />
</div>
</form>
<%
...
<div id="ListNav">
    <ul>
        <li><a href="javascript:void(0);" onclick="ShowBox('FormMaker.asp?Type=9&id=<%=FormId%>');">添加字段</a></li>
        <li><a href="javascript:void(0);" onclick="SetRContent('MainRight','<%=Session("NowPage")%>');return false">刷新</a></li>
    </ul>
</div>
<div id="ListContent">
    ...
    <table width="100%" bordercolor="#CCCCCC" border="1" cellspacing="0" cellpadding="0">
        <tr>
            <td align="center" class="ListTdTop">字段名称</td>
            <td align="center" class="ListTdTop">字段别名</td>
            <td align="center" class="ListTdTop">调用标签</td>
            <td align="center" class="ListTdTop">字段类型</td>
            <td align="center" class="ListTdTop">是否必填</td>
...
%></td>
    <td align="center"><%if Rs("IsNotNull")=0 Then Response.Write "是" Else Response.Write "否" %></td>
 	<td align="center">
          <input name="OrderID" type="text"  class="Input"  id="OrderID" value="<%=rs("OrderID")%>" size="4" maxlength="3">
          <input name="A" type="hidden" id="A" value="N">
		  <input name="ID" type="hidden" id="ID" value="<%=rs("ID")%>">	
           
	</td>
	<td align="center"><a href="javascript:void(0);" onclick="ShowBox('FormMaker.asp?Type=10&id=<%=Rs("id")%>&tbname=<%=tbname%>');">修改</a> ┆ 

<a href="javascript:void(0);" onClick="DelIt('您确认要删除该字段吗，此操作不可逆！','FormMaker.asp?Type=13&Id=<%=Rs("id")%>&tbname=<%=tbname%>&FieldName=<%=Rs("FieldName")%>','MainRight','<%=Session("NowPage")%>');return false;">删除</a>  </td>
  </tr>
<%
			Rs.MoveNext
			i=i+1
...
<div id="BoxTop" style="width:98%;"><span><%if Types=9 then response.write "添加字段":else:response.write "修改字段"%></span><a onclick="$('#Boxs').hide();$('select').show();"><img src="images/close3.gif"></a></div>
<div id="BoxContents" style="width:98%;">
<table class="table" cellspacing="1" cellpadding="2" width="98%" border="0" align="center">
  ...
  <td align="right"> 是否必填： </td>
    <td><table id="IsNotNull" border="0">
      <tr>
        <td><input id="IsNotNull_0" <% IF IsNotNull = "0" Then Response.Write "Checked" %> type="radio" name="IsNotNull" value="0" style="vertical-align:center;"/><label for="IsNotNull_0" style="vertical-align:center;">是</label></td>
        <td><input id="IsNotNull_1" <% IF IsNotNull = "1" Then Response.Write "Checked" %> type="radio" name="IsNotNull" value="1" style="vertical-align:center;"/><label for="IsNotNull_1" style="vertical-align:center;">否</label>
			  </td>
      </tr>
    </table></td>
  </tr>
...
<table id="FieldType" <%If Types=10 Then Response.write " disabled=""disabled"" "%> onClick="SelectModelType()" border="0">
	<tr>
		<td><input id="Type_0" <% IF FieldType = "TextType" Then Response.Write "Checked" %> type="radio" name="FieldType" value="TextType" checked="checked"  style="vertical-align:center;"/>
		<label for="Type_0" style="vertical-align:center;">单行文本</label></td>
		<td><input id="Type_1" <% IF FieldType = "MultipleTextType" Then Response.Write "Checked" %>  type="radio" name="FieldType" value="MultipleTextType"  style="vertical-align:center;"/>
		<label for="Type_1" style="vertical-align:center;">多行文本(不支持Html)</label></td><td>
		...
		<label for="Type_2" <% IF FieldType = "SupportHtmlType" Then Response.Write "Checked" %>  style="vertical-align:center;">多行文本(支持Html)</label></td><td>
		...
		<td><input id="Type_4" <% IF FieldType = "ListBoxType" Then Response.Write "Checked" %>  type="radio" name="FieldType" value="ListBoxType"  style="vertical-align:center;"/><label for="Type_4" style="vertical-align:center;">多选项</label></td>
	</tr><tr>
		<td><input id="Type_5" <% IF FieldType = "DateType" Then Response.Write "Checked" %>  type="radio" name="FieldType" value="DateType"  style="vertical-align:center;"/>
		<label for="Type_5" style="vertical-align:center;">日期时间</label></td><td>
		...
		<label for="Type_6" style="vertical-align:center;">上传</label></td><td><input <% IF FieldType = "FileType" Then Response.Write "Checked" %>  id="Type_7" type="radio" name="FieldType" value="FileType"  style="vertical-align:center;"/>
		<label for="Type_7" style="vertical-align:center;">下载</label></td><td><input <% IF FieldType = "NumberType" Then Response.Write "Checked" %>  id="Type_8" type="radio" name="FieldType" value="NumberType"  style="vertical-align:center;"/>
		<label for="Type_8" style="vertical-align:center;">数字</label></td><td><input <% IF FieldType = "RadomType" Then Response.Write "Checked" %>  id="Type_9" type="radio" name="FieldType" value="RadomType"  style="vertical-align:center;"/>
...
<div id="BoxBottom" style="width:96%;">
		...
		<input type="hidden" name="ID" value="<%=ID%>" />
        <input type="submit" onclick="if(CheckForm()){Sends('FormFieldAdd','FormMaker.asp?Type=<%=A%>',0,'',0,1,'MainRight','<%=Session("NowPage")%>');}" class="Button" name="button" id="button" value="<%if Types=9 then response.write "添 加":else:response.write "修 改"%>" />
        <input type="button" onclick="$('#Boxs').hide();$('select').show();" class="Button" name="btnClose" id="btnClose" value="关 闭" />
</div>
	</form>
<script language="JavaScript" type="text/javascript">
