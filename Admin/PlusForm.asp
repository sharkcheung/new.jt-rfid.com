...
End Select
 ...
 conn.execute("update "&Ext_Table_Name&"_Form set viewstatu=1 where id="&id&"")
	 ...
	 %>
<div id="BoxTop" style="width:98%;"><span>详细信息</span></div>
...
<div id="ListNav">
    <ul>
        <li><a href="javascript:void(0);" onclick="SetRContent('MainRight','PlusForm.asp?Type=1&modeid=<%=modeid%>');return false">刷新内容</a></li>
    </ul>
</div>
<div id="ListTop">
    【<%=Ext_Form_Name%>】模块
...
%>
        <tr>
            ...
            <td align="center"><a href="javascript:void(0);" onclick="ShowBox('PlusForm.asp?Type=2&modeid=<%=modeid%>&Id=<%=Rs("id")%>');">查看详情</a> <a href="javascript:void(0);" onclick="DelIt('您确认要删除？此操作不可逆！','PlusForm.asp?Type=4&modeid=<%=modeid%>&Id=<%=Rs("id")%>','MainRight','<%=Session("NowPage")%>');">删除</a></td>
        </tr>
<%
			Rs.MoveNext
			i=i+1
...
%>
        <tr>
            <td height="30" colspan="4">&nbsp;<%Call FKFun.ShowPageCode("PlusForm.asp?Type=1&modeid="&modeid&"&Page=",PageNow,PageAll,PageSizes,PageCounts)%></td>
        </tr>
<%
	Else
%>
