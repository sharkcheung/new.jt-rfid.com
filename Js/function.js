...
function ShowBox(DoUrl){
	document.getElementById('BoxContent').innerHTML="<div id='LoadBox'><a href='javascript:void(0);' title='点击关闭' onclick=$('select').show();$('#Boxs').hide()><img src='Images/Loading.gif' /></a></div>";
	$("#Boxs").show();
	$.get(DoUrl,
		function(data){
...
function DelIt(Cstr,Urls,F5Div,F5Url){
	if(confirm(Cstr)){
		$.get(Urls,
			function(data){
				//alert(data);
				ymPrompt.alert({title:'提示！',message:data,width:350,height:200})
    ...
    var options = { 
        ...
        success:function(responseText, statusText){
          if(statusText=="success"){
           ...
           // alert(statusText);
		   ymPrompt.alert({title:'提示！',message:statusText,width:350,height:200})
          }
		}
    }; 
        ...
        success:function(responseText, statusText){
          if(statusText=="success"){
            if(responseText.search("成功")>0){
				if(SuAlert==1){
					//alert(responseText);
					ymPrompt.alert({title:'提示！',message:responseText,width:350,height:200})
				}
				else{
					$("#Boxs").hide();
					var st=responseText.replace(/\|\|\|\|\|/gi,"\n");
					//alert(st);
					ymPrompt.alert({title:'修改成功！',message:st,width:350,height:200})
				}
				if(SuGo==1){
					location.href=GoUrl;
				}
				...
				var st=responseText.replace(/\|\|\|\|\|/gi,"\n");
				//alert(st);
				ymPrompt.alert({title:'错误提示！',message:st,width:350,height:200})
		 		 $("#button").val(oldval);
...
xml.send();
   ...
   {
      //alert ("获取失败，请重试！");
	ymPrompt.alert({title:'自动获取失败！',message:'获取失败或该账号未设置开通，请重试！',width:300,height:180})
   }
   else
   {
