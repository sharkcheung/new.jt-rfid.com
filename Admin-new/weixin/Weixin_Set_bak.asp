...
<div id="BoxTop" style="width:98%;"><span>微信接口设置</span></div>
...
1. 请先登录 <a href='https://mp.weixin.qq.com/' target="_blank"><span style='color: #0C0;'>微信公众平台</span></a> 如果没有，请先<a href="https://mp.weixin.qq.com/cgi-bin/readtemplate?t=register/step1_tmpl&lang=zh_CN" target="_blank"><span style='color: #0C0;'>开通公众账号</span></a> 
2. 请设置好以下必填项(带"*"号为必填项)，并点击保存设置
3. 进入[高级功能]，在关闭[编辑模式]后，开启并进入[开发模式]配置页面 
4. 把下列的接口配置信息 <span style='color: #0C0;'>URL</span> 和 <span style='color: #0C0;'>Token</span> 写入微信公众平台的配置页面后提交即可 
5. 如果您的公众号拥有高级接口权限，请将公众平台中的 <span style='color: #0C0;'>开发者凭据(AppId和AppSecret)</span> 设置在此页面</pre>
	   </div>
	<table width="90%" border="0" align="center" id="table001"  cellpadding="0" cellspacing="0">
        <tr>
            ...
            <td><input name="wx_url" type="text" class="Input" id="wx_url" value="http://<%=Request.ServerVariables("Http_Host")%>/admin/weixin/" size="50"  style="vertical-align:middle" readonly/></td>
            <td><span style='color: #f00;'>*</span> 此项由系统自动生成</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><span style='color: #f00;'>*</span> 即微信后台账户信息中显示的原始ID</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><textarea name="wx_NoneReply" id="wx_NoneReply" rows="4" cols="35" readonly><%=wx_NoneReply%></textarea> <br><input type="button" value="回复文本" class="Button addNoneText"> <input type="button" value="回复图文" class="Button addNoneNews"></td>
            <td>注：用户关注您的微信账号后将收到此内容，留空则不做处理</td>
        </tr>
        <tr>
...
<div id="BoxBottom" style="width:96%;">
        <input type="submit" onclick="Sends('SystemSet','/admin/weixin/Weixin_Set.asp?Type=2',0,'',1,0,'','');" class="Button" name="button" id="button" value="保存设置" />
