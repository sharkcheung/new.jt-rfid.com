...
.layui-layer input.Input, input.Input.zffs{padding-left:0px;}
</style>
<div id="ListTop" style="text-indent:0;padding:10px;">请选择栏目：
<select name="D1" id="D1" style="vertical-align:middle;">
      <option value="alert('请选择栏目');">请选择栏目</option>
<%
Call ModuleSelectUrl(1,0,Fk_Module_Id)
%>
</select>&nbsp;<input name="SearchStr" value="<%=SearchStr%>" type="text" class="Input" id="SearchStr" style="vertical-align:middle;" size=10/>&nbsp;<input type="button" class="Button" onclick="SetRContent('MainRight','Weixin_GetArticle.asp?Type=1&ModuleId=<%=Fk_Module_Id%>&SearchStr='+escape(document.all.SearchStr.value));" name="S" Id="S" value="  查询  "  style="vertical-align:middle;"/>
</div>
