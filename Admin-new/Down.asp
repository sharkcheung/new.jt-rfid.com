...
Select Case Types
	...
	Case 9
		Call DownOrderDo() '执行下载栏目内容批量排序
	Case 10
		Call DownRelationList() '关联资料下载列表
	Case Else
...
<div id="ListContent">
	<div class="gnsztopbtn">
    	<h3><%=Fk_Module_Name%>模块</h3><input name="SearchStr" value="<%=SearchStr%>" type="text" class="Input" id="SearchStr" style="vertical-align:middle;"/>&nbsp;<input type="button" class="Button" onclick="SetRContent('MainRight','Down.asp?Type=1&ModuleId=<%=Fk_Module_Id%>&SearchStr='+escape(document.all.SearchStr.value));" name="S" Id="S" value="  查询  "  style="vertical-align:middle;"/>
        <span style="line-height:26px; float:left; padding-left:20px;">请选择模块：</span>
...
%>
		...
		<a  class="tjia" href="javascript:void(0);" onclick="ShowBox('Down.asp?Type=2&ModuleId=<%=Fk_Module_Id%>','添加','1000px','500px');">添加</a>
        <a class="shuax" href="javascript:void(0);" onclick="SetRContent('MainRight','<%=Session("NowPage")%>');">刷新</a>
    </div>
    <form name="DelList" id="DelList" method="post" action="Down.asp?Type=7" onsubmit="return false;">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <th width="60" align="center" class="ListTdTop">选</th>
            <th width="350" align="center" class="ListTdTop">下载名称</th>
            <th align="center" style="display:none" class="ListTdTop">文件名</th>
            <th align="center" class="ListTdTop">下载参数</th>
            <th align="center" class="ListTdTop">点击量</th>
            <th align="center" class="ListTdTop">排序</th>
            <th width="160" align="center" class="ListTdTop">添加时间</th>
...
%>
        <tr>
            <td height="20" align="center"><input type="hidden" value="<%=Rs("Fk_Down_Id")%>" name="id[]"><input type="checkbox" name="ListId" class="Checks" value="<%=Rs("Fk_Down_Id")%>" id="List<%=Rs("Fk_Down_Id")%>" /></td>
            <td><span style="line-height:22px; padding-right:15px"><%=Rs("Fk_Down_Title")%><%If Rs("Fk_Down_Color")<>"" Then%><span style="color:<%=Rs("Fk_Down_Color")%>">■</span><%End If%><%If Rs("Fk_Down_Url")<>"" Then%>[转向链接]<%End If%></span></td>
            <td align="center" style="display:none"><%=Rs("Fk_Down_FileName")%></td>
            <td align="center"><%If Rs("Fk_Down_Show")=1 Then%><span style="color:#000">[显]</span><%Else%><span style="color:#CCC">[隐]</span><%End If%><%If Rs("Fk_Down_Pic")<>"" Then%><span style="color:#F00">[图]</span><%End If%><span title="<%=Fk_Down_Template%> ">[模]</span><%If InStr(Fk_Down_Recommend,"推荐")>0 Then%><span title="<%=Replace(Fk_Down_Recommend,",","")%> ">[推]</span><%End If%><%If trim(Rs("Fk_Down_Ip")&" ")="1" Then%><span title="置顶 "><font color=blue style="font-weight:bolder;">[顶]</font></span><%End If%><%If Fk_Down_Subject<>"" Then%><span title="<%=Fk_Down_Subject%> ">[专]</span><%End If%></td>
            <td align="center"><%=Rs("Fk_Down_Click")%></td>
            <td height="20" align="center"><input type="text" value="<%=Rs("Px")%>" name="Fk_Down_px[]" style="width:25px;text-align:center"/></td>
            <td align="center"><%=Rs("Fk_Down_Time")%></td>
            <td align="center" class="no6">
            <a class="no2" href="javascript:void(0);" onclick="ShowBox('Down.asp?Type=4&ModuleId=<%=Fk_Module_Id%>&Id=<%=Rs("Fk_Down_Id")%>','修改','1000px','500px');"></a>
            <a class="no4" href="javascript:void(0);" onclick="DelIt('您确认要删除“<%=Rs("Fk_Down_Title")%>”，此操作不可逆！','Down.asp?Type=6&Id=<%=Rs("Fk_Down_Id")%>','MainRight','<%=Session("NowPage")%>');"></a></td>
        </tr>
<%
			Rs.MoveNext
			i=i+1
...
%>
        <tr>
            <td height="30" colspan="8" style="padding-left:22px;" class="NowPage">
            <input name="chkall" type="checkbox" id="chkall" value="select" onClick="CheckAll(this.form)">&nbsp;全选&nbsp;&nbsp;
            <input style="vertical-align:middle" type="submit" value="删 除" class="Button" onClick="DelIt('确定要删除选中的下载吗？','Down.asp?Type=7&ListId='+GetCheckbox(),'MainRight','<%=Session("NowPage")%>');">
			<input type="submit" value="更新排序" class="Button" onClick="Sends('DelList','Down.asp?Type=9',0,'',0,1,'MainRight','<%=Session("NowPage")%>');" style="vertical-align:middle;">
...
'==========================================
'函 数 名：DownRelationList()
'作    用：关联下载资料列表
'参    数：
'==========================================
Sub DownRelationList()
	'新功能，追加SEO title字段
	'2017年5月22日
	'middy241@163.com
	if CheckFields("Fk_Down_seotitle","Fk_Down")=false then
	...
	Dim SearchStr
	SearchStr=FkFun.HTMLEncode(Trim(Request.QueryString("SearchStr")))
	Dim selectedIds,arrId
	selectedIds=FkFun.HTMLEncode(Trim(Request.QueryString("selectedIds")))
	arrId = split(selectedIds,",")
...
<div id="ListContentRelated">

	<div class="gnsztopbtn">
    	<input name="SearchStr" placeholder="请输入要搜索的下载名称" value="<%=SearchStr%>" type="text" class="Input" id="SearchStr" style="vertical-align:middle;width: 220px;margin-left:20px;"/>&nbsp;<input type="button" class="Button" onclick="SetRContent('MainRight','Down.asp?Type=1&ModuleId=<%=Fk_Module_Id%>&SearchStr='+escape(document.all.SearchStr.value));" name="S" Id="S" value="  查询  "  style="vertical-align:middle;"/>
    </div>
    <form name="DelList" id="DelList" method="post" action="Down.asp?Type=7" onsubmit="return false;">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
...
<%
	Dim Rs2
	Set Rs2=Server.Createobject("Adodb.RecordSet")
	Sqlstr="Select * From [Fk_Down] Where Fk_Relation_Product_Id is null"
	If SearchStr<>"" Then
		Sqlstr=Sqlstr&" And Fk_Down_Title Like '%%"&SearchStr&"%%'"
	End If
	If selectedIds<>"" Then
		Sqlstr=Sqlstr&" And Fk_Down_Id Not In ("&selectedIds&")"
	End If
	Sqlstr=Sqlstr&" Order By Fk_Down_Id Desc"
	Rs.Open Sqlstr,Conn,1,1
	If Not Rs.Eof Then
...
%>
        <tr>
            <td height="20" align="center"><input type="hidden" value="<%=Rs("Fk_Down_Id")%>" name="id[]"><input type="checkbox" <%if inArray(Rs("Fk_Down_Id"), arrId) then%> checked <% end if%> name="ListId" class="Checks" value="<%=Rs("Fk_Down_Id")%>" id="List<%=Rs("Fk_Down_Id")%>" /><input type="hidden" value="<%=Rs("Fk_Down_Title")%>" name="title[]"></td>
            <td><span style="line-height:22px; padding-right:15px"><%=Rs("Fk_Down_Title")%></span></td>
            <td align="center"><%=Rs("Fk_Down_Click")%></td>
            <td align="center"><%=Rs("Fk_Down_Time")%></td>
...
%>
        <tr>
            <td height="30" colspan="4" style="padding-left:22px;" class="NowPage">
            <input name="chkall" type="checkbox" id="chkall" value="select" onClick="CheckAll(this.form)">&nbsp;全选&nbsp;&nbsp;
			<%Call FKFun.ShowPageCodeRelated("Down.asp?Type=10&selectedIds="&Server.URLEncode(selectedIds)&"&SearchStr="&Server.URLEncode(SearchStr)&"&Page=",PageNow,PageAll,PageSizes,PageCounts)%>
			</td>
        </tr>
<%
	Else
%>
        <tr>
            <td height="25" colspan="4" align="center">暂无记录</td>
        </tr>
<%
	End If
	Rs.Close
...
$(document).ready(function(){
	...
	if(window.KindEditor){
		if(("#Fk_Down_File").length>0){
			$("#Fk_Down_File").after("<input type=\"button\" id=\"uploadButton\" value=\"上传\" class=\"Button\"/>*上传限于rar/zip/doc/xls/ppt/pdf格式,文件最大允许5M");
...
<div id="BoxContents" style="width:93%; padding-top:20px;">
    ...
    <tr>
        <td height="28" align="right" width="100">下载名称：</td>
        <td><input name="Fk_Down_Title"<%If SiteToPinyin=1 Then%> onchange="GetPinyin('Fk_Down_FileName','ToPinyin.asp?Str='+this.value);"<%End If%> type="text" class="Input" id="Fk_Down_Title" size="50"  style="vertical-align:middle;"/> <input name="Fk_Down_Color" type="hidden" id="Fk_Down_Color" value=""/><span class="colorpicker" onclick="colorpicker('colorpanel_title','set_title_color');" title="标题颜色" style="vertical-align:middle;background: url(http://image001.dgcloud01.qebang.cn/website/ext/images/icon/color.png) 0px 0px no-repeat;height: 24px;width: 24px;display: inline-block;"></span>
                           <span class="colorpanel" id="colorpanel_title" style="position:absolute;z-index:99999999;"></span><script type="text/javascript">
						   set_title_color("");
                           </script></td>
    	<td align="right" width="100" colspan="2"> 排序：</td>
        <td><input style="width:60px" type="text" value="0" id="Fk_Down_px" name="Fk_Down_px" class="Input"  size="4" maxlength="6" style="vertical-align:middle;"/>（限数字，越大越前）</td>
    </tr>
    <tr>
        ...
        <td><input  style="width:60px" name="Fk_Down_Description" type="text" class="Input" id="Fk_Down_Description" size="30" style="vertical-align:middle;" /> <input type="submit" onclick="tiqu(1,'Fk_Down_Content','Fk_Down_Description');" class="Button" name="btntqdesc" id="btntqdesc" value="提 取"  style="vertical-align:middle;"/></td>
    </tr>
    <tr>
        ...
        <td><input  style="width:60px" name="Fk_Down_Language" value="简体中文" type="text" class="Input" id="Fk_Down_Language" size="30"  style="vertical-align:middle;"/></td>
    </tr>
    <tr style="display:none">
        ...
        <td colspan="2">
		<font color="red">说明：只需上传一张图，自动生成缩略小图和正常大图。</font></td>
    </tr>
    <tr>
...
%>
    ...
    <tr>
        ...
        <td colspan="4" style="padding:10px;"><textarea name="Fk_Down_Content" class="<%=bianjiqi%>" id="Fk_Down_Content" rows="8" style="width:100%;"></textarea></td>
    </tr>
     <tr>
...
%>
        ...
        <input type="radio" name="Fk_Down_Show" class="Input" id="Fk_Down_Show1" value="0"  style="vertical-align:middle;"/><label for="Fk_Down_Show1" style="vertical-align:middle;">不显示</label>　推荐：<select name="Fk_Down_Recommend" class="Input" size="1" id="Fk_Down_Recommend" style="vertical-align:middle;">
            ...
            </select> &nbsp; &nbsp; 下载量：<input style="width:50px" name="Fk_Down_click" type="text" id="Fk_Down_click" class="Input" size="6" maxlength="6" value="<%=rnd_num%>" style="vertical-align:middle;"></td>
    </tr>
    </table>
</div>
...
$(document).ready(function(){
	...
	if(window.KindEditor){
		if(("#Fk_Down_File").length>0){
			$("#Fk_Down_File").after("<input type=\"button\" id=\"uploadButton\" value=\"上传\" class=\"Button\"/>*上传限于rar/zip/doc/xls/ppt格式,文件最大允许5M");
				var editor = window.KindEditor.editor({
						fileManagerJson : '/admin/dkidtioenr/aps/file_manager_json.asp',
						uploadJson		: '/admin/dkidtioenr/aps/upload_json.asp?dir=file',
						allowFileManager : true
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
        <td height="28" align="right" width="100">名称：</td>
        <td><input name="Fk_Down_Title"<%If SiteToPinyin=1 And (Fk_Down_FileName="" Or IsNull(Fk_Down_FileName)) Then%> onchange="GetPinyin('Fk_Down_FileName','ToPinyin.asp?Str='+this.value);"<%End If%> value="<%=Fk_Down_Title%>" type="text" class="Input" id="Fk_Down_Title" size="50"  style="vertical-align:middle"/> <input name="Fk_Down_Color" type="hidden" id="Fk_Down_Color" value=""/><span class="colorpicker" onclick="colorpicker('colorpanel_title','set_title_color');" title="标题颜色" style="vertical-align:middle;background: url(http://image001.dgcloud01.qebang.cn/website/ext/images/icon/color.png) 0px 0px no-repeat;height: 24px;width: 24px;display: inline-block;"></span>
                           <span class="colorpanel" id="colorpanel_title" style="position:absolute;z-index:99999999;"></span><script type="text/javascript">
						   set_title_color("<%=Fk_Down_Color%>");
                           </script></td>
    	<td  colspan="2" width="100" align="right">排序：</td>
        <td><input style="width:60px" type="text" name="Fk_Down_px" id="Fk_Down_px" class="Input" value="<%if isnull(Fk_Down_px) then response.write "0" else response.write Fk_Down_px%>" maxlength="6" size="4" style="vertical-align:middle"/>（限数字,越大越前）&nbsp; &nbsp; <input type="checkbox" name="Fk_Down_Time" id="Fk_Down_Time" value="1"  style="vertical-align:middle"/><label for="Fk_Down_Time" style="vertical-align:middle">&nbsp;更新时间</label></td>
    </tr>
    <tr>
        <td height="28" align="right">关键字：</td>
        ...
        <td align="right" colspan="2">描述：</td>
        <td><input  style="width:60px" name="Fk_Down_Description" value="<%=Fk_Down_Description%>" type="text" class="Input" id="Fk_Down_Description" size="30"  style="vertical-align:middle"/> <input type="submit" onclick="tiqu(1,'Fk_Down_Content','Fk_Down_Description');" class="Button" name="btntqdesc" id="btntqdesc" value="提 取"  style="vertical-align:middle"/></td>
    </tr>
    <tr>
        <td height="28" align="right">适用系统：</td>
        ...
        <td align="right" colspan="2">语言：</td>
        <td><input  style="width:60px" name="Fk_Down_Language" value="<%=Fk_Down_Language%>" type="text" class="Input" id="Fk_Down_Language" size="30"  style="vertical-align:middle"/></td>
    </tr>
    <tr style="display:none">
        <td height="28" align="right">题图：</td>
...
%>
    ...
    <tr>
        <td height="28" align="right" width="100">下载内容：</td>
        <td colspan="4" style="padding:10px;"><textarea name="Fk_Down_Content" class="<%=bianjiqi%>" id="Fk_Down_Content" rows="8" style="width:100%;"><%=Fk_Down_Content%></textarea></td>
    </tr>
    <tr>
        <td height="28" align="right">模板：</td>
        <td colspan="4"><select name="Fk_Down_Template" class="Input" id="Fk_Down_Template" style="vertical-align:middle">
...
%>
        ...
        <input type="radio" name="Fk_Down_Show" class="Input" id="Fk_Down_Show1" value="0"<%=FKFun.BeCheck(Fk_Down_Show,0)%>  style="vertical-align:middle"/><label for="Fk_Down_Show1" style="vertical-align:middle">不显示</label> &nbsp; &nbsp; 下载量：<input style="width:50px;" name="Fk_Down_click" type="text" id="Fk_Down_click" class="Input" size="6" maxlength="6" value="<%=Fk_Down_click%>" style="vertical-align:middle;"></td>
    </tr>
    </table>
</div>
<div id="BoxBottom" style="width:93%; margin:0 auto;" class="tcbtm">
...
Sub DownOrderDo()
	...
	Fk_Down_px=Replace(Trim(Request.Form("Fk_Down_px[]"))," ","")
	dim idarr
	If Id="" Then
	...
	comma=""
	if instr(Id,",")>0 and instr(Fk_Down_px,",")>0 then
		arrId = split(Id,",")
		arrPx = split(Fk_Down_px,",")
		if ubound(arrId)=ubound(arrPx) then
			for i = 0 to ubound(arrId)
				Call FKFun.ShowNum(arrPx(i),"排序序号必须是有效数字！")
				Sqlstr="Select Px From [Fk_Down] Where Fk_Down_Id="&arrId(i)&""
				Rs.Open Sqlstr,Conn,1,3
				if not Rs.eof then
					Application.Lock()
						Fk_Down_Title=Fk_Down_Title & comma&"【"&arrId(i)&"】"
						comma = ","
						Rs("Px")=arrPx(i)
						Rs.Update()
					Application.UnLock()
	...
	elseif instr(Id,",")=0 and instr(Fk_Down_px,",")=0 and Id>0 then
		Call FKFun.ShowNum(Fk_Down_px,"排序序号必须是有效数字！")
		Sqlstr="Select Px,Fk_Down_Title From [Fk_Down] Where Fk_Down_Id="&Id&""
		Rs.Open Sqlstr,Conn,1,3
		if not Rs.eof then
			Application.Lock()
				Fk_Down_Title="【"&Id&"】"
	...
	on error resume next
	dim log_content,log_ip,log_user
	log_content="批量更新下载栏目下ID为"&Fk_Down_Title&"的内容排序"
	log_user=Request.Cookies("FkAdminName")
