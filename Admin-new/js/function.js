...
function OpenBox(DoUrl,strTitle){
			...
			if($(".kinediter").length>0){	
				...
				{					
					KindEditor.create('.kinediter', {
						themeType		: 'simple',
						uploadJson		: '/'+adminpath+'/dkidtioenr/aps/upload_json.asp',
						fileManagerJson : '/'+adminpath+'/dkidtioenr/aps/file_manager_json.asp',
						...
						htmlTags		:{
								...
								hr : ['class', '/'],
								br : ['/'],
						        'p,ol,ul,li,blockquote,h1,h2,h3,h4,h5,h6,pre' : ['align', 'style'],
...
function OpenBoxNew(DoUrl,strTitle, sbwidth, sbheight, btnTitle, selectedIds, id, name){
	...
	$.get(DoUrl + "&selectedIds=" + selectedIds.join(','),
		function(data){
			layer.closeAll('loading');
			//document.getElementById('BoxContent').innerHTML=data;
			var layopen=layer.open({
				...
				yes: function(index, layero){ //或者使用btn1
					if(layero.find("input.Checks:checked").length>0){
						let arr = [];
						$("input.Checks:checked").each(function (i) {
							arr[i] = { id: $(this).val(), title: $(this).next('input').val()};
							$("#"+id).append("<li><input type='hidden' value='"+$(this).val()+"' name='"+name+"'/><span>"+$(this).next('input').val()+"</span><span class='remove-related'><a href='javascript:void(0);'>移除</a></span></li>")
						})
						console.log(arr);
						$("#"+id + " .remove-related a").on('click',function(){
							console.log(arr)
							let __this = $(this);
							let newArr = arr.filter((item, i, arr) => {
							//函数本身返回布尔值，只有当返回值为true时，当前项存入新数组。
								console.log(item.title,__this.parent().parent('li').find('span').eq(0).text())
								return item.title!=__this.parent().parent('li').find('span').eq(0).text();
							})
							console.log(newArr);
...
//==========================================
//函数名：SetRContentRelated
//用途：替换DIV内容;
//参数：DivId：&#59110;替换的DIV
//     Urls：获取内容的链接
//==========================================
function SetRContentRelated(DivId,Urls){
	console.log(DivId)
	$.get(Urls,
		function(data){
...
//     F5Div：刷新DIV
//==========================================
function SendsNotClose(FormName,ToUrl){
	var oldval=$("#noclosebutton").val();
    var options = { 
		url:  ToUrl,
        beforeSubmit:function(formData, jqForm, options){
		  $("#noclosebutton").val("正在提交...");
		  $("#noclosebutton").attr("disabled","disabled");
          return true; 
		},
        success:function(responseText, statusText){
          if(statusText=="success"){
            if(responseText.search("成功")>0){	
				if($("#wx_Subscribeval").length>0){
					$("#wx_Subscribe").val($("#wx_Subscribeval").val());
				}
				if($("#wx_NoneReplyval").length>0){
					$("#wx_NoneReply").val($("#wx_NoneReplyval").val());
				}
				if($("input.wx_NoneReplyChecks:checked").length>0){
					var c;
					c=$("input.wx_NoneReplyChecks:checked").val();
					$("#wx_NoneReply").val("[wx_news=\""+c+"\"]");
				}
				if($("input.wx_SubscribeChecks:checked").length>0){
					var c;
					c=$("input.wx_SubscribeChecks:checked").val();
					$("#wx_Subscribe").val("[wx_news=\""+c+"\"]");
				}
				var st=responseText.replace(/\|\|\|\|\|/gi,"\n");
				//alert(st);
				layer.msg(st);
		 		 $("#noclosebutton").val(oldval);
		 		 $("#noclosebutton").removeAttr("disabled");
            }
          }
          else{
            //alert(statusText);
...
var xml = new ActiveXObject("MSXML2.XMLHTTP");
    ...
    xml.open("get","/'+adminpath+'/GetData.asp?act="+auto,false);
xml.send();
...
//mainStr.length 
 ...
 function chakeywordspaiming(keywords,i){
	$("#chaciarea"+i).html("<img src=/'+adminpath+'/shangwin/images/load.gif>");
	$.ajax({
		type:"GET",
		url:"/'+adminpath+'/shangwin/getPaiming.asp",
...
//alert(domainname.length);
 ...
 function savekeywordspaiming(keywords,paimingjieguo){
	...
	// xml.send();
	$.ajax({
			url: '/'+adminpath+'/saveseopaiming.asp',
			type: 'GET',
			dataType: 'html',
			cache: false,
			timeout: 10000,
...
function GetRank(u,k,i){
	$.ajax({
		type:"GET",
		url:"/'+adminpath+'/shangwin/seo/paiming/getRank.asp",
		...
		beforeSend:function(){
			$("#chaciarea"+i).html("<img src=/'+adminpath+'/shangwin/seo/paiming/images/load.gif>");
		},
		success:function(msg){
			$("#chaciarea"+i).html(msg);
		}
...
function updatekeyword(u,k){
	$.ajax({
		type:"GET",
		url:"/'+adminpath+'/shangwin/updatePVR.asp",
...
function GetVisits(tjid,keyword,i,n){
	$.ajax({
		type:"GET",
		url:"/'+adminpath+'/shangwin/xmlhttp.asp",
		data:"tjid="+tjid+"&k="+encodeURIComponent(keyword)+"&r="+Math.random(),
		dataType:"html",
		cache: false,
		timeout: 90000,
		beforeSend:function(){
			$("#chavisits"+i).html("<img src=/'+adminpath+'/shangwin/images/load.gif>");
		},
		error: function(){
...
function GetAllVisits(tjid){
	$("#Allvisits").html("<img src=/'+adminpath+'/shangwin/images/load.gif>");
	$.ajax({
		type:"GET",
		url:"/'+adminpath+'/shangwin/GetAllVisits.asp",
