...
Sub ModuleList()
	...
	Rs.Close
%>
<div class="gnsztopbtn"><a title="添加新栏目 " class="sixjis" style="width:90px; padding-left:30px;" href="javascript:void(0);" onclick="ShowBox('Module.asp?Type=2&MenuId=<%=MenuId%>','添加新栏目');">添加新栏目</a><a title="栏目排序 " class="no2" href="javascript:void(0);" onclick="SetRContent('MainRight','Module.asp?Type=7&MenuId=<%=MenuId%>');return false;">栏目排序</a><%If FkFun.CheckLimit("System5") Then%><a title="更新缓存 " class="no3" href="javascript:void(0);" onclick="DelIt('需要更新缓存么？','Template.asp?Type=6','MainRight','<%=Session("NowPage")%>');return false;">更新缓存</a><%End If%></div>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
        <td  width="100" align="right">栏目类型：</td>
        <td><select style="width:120px" name="Fk_Module_Type" class="Input" id="Fk_Module_Type" onchange="ModuleTypeChange(this.options[this.options.selectedIndex].value);">
        <%if Lmlx<>"" then%>
	...
	<tr>
        <td height="28" align="right" valign="top">栏目图片：</td>
        <td>
				...
				<table border="0" cellpadding="0" cellspacing="0">
					...
					<tr>
						<td valign="top" style="padding:0px;border-bottom:0px;">
							...
							<img class="img" id="SlImg" style="width:120px;" src="http://image001.dgcloud01.qebang.cn/website/ext/images/image.jpg">                                  </div>                              
						</td>
						<td valign="top" style="padding:0px 0px 0px 6px;border-bottom:0px;">
							<input type="text" class="Input" id="Fk_Module_Pic" name="Fk_Module_Pic" style="width:314px; " value="">
							<div style="padding:5px 0px 0px 10px;">
							<span class="sysBtn" id="UploadImage"><a id="chooseImgModule" style="cursor:pointer;display: inline-block;width: 100px;" class="Button"><b class="icon icon_add"></b>添加图片...</a></span>                                  
							</div>
    ...
    <tr>
        <td align="right">栏目分级：</td>
        <td><select style="width:120px;" name="Fk_Module_Level" class="Input" id="Fk_Module_Level">
...
%>
    ...
    <tr id="Fk_Module_PageCounts" style="display:none;">
        ...
        <td><select  style="width:120px;" name="Fk_Module_PageCount" class="Input" id="Fk_Module_PageCount">
...
%>
    ...
    <tr id="Fk_Module_PageCodes" style="display:none;">
        <td align="right" <%If Request.Cookies("FkAdminLimitId")<>0  Then%> style="display:none;"<%End If%>>页码字符：</td>
    ...
    <tr id="Fk_Module_Templates">
        <td align="right" <%If Request.Cookies("FkAdminLimitId")<>0  Then%> style="display:none;"<%End If%>>显示模板：</td>
...
%>
    ...
    <tr id="Fk_Module_LowTemplates" style="display:none;">
        <td align="right" <%If Request.Cookies("FkAdminLimitId")<>0  Then%> style="display:none;"<%End If%>>子内容模板：</td>
...
<div id="BoxBottom" style="width:93%; margin:0 auto; text-align:left" class="tcbtm">
        ...
        <input style="margin-left:113px;" type="submit" onclick="Sends('ModuleAdd','Module.asp?Type=3',0,'',0,1,'MainRight','Module.asp?Type=1&MenuId=<%=MenuId%>');" class="Button" name="button" id="button" value="添 加" />
...
Sub ModuleAddDo()
	...
	Fk_Module_Pic=Trim(Request.Form("Fk_Module_Pic"))
...
$(document).ready(function(){
	$("#extMutiImgjs").attr("src","ext/js/extMutiImg.js?r="+Math.random());
	<%if Fk_Module_Pic<>"" then %>
	$('#SlImg').attr("src","<%=Fk_Module_Pic%>");
	<%end if%>
})
</script>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
        <td align="right" width="100">栏目类型：</td>
...
%>
			...
			<div class="image_grid" style="padding:10px;">
				<table border="0" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<td valign="top" style="padding:0px 0px 0px 6px;border-bottom:0px;">
							<input type="text" class="Input" id="Fk_Module_Pic" name="Fk_Module_Pic" style="width:314px; " value="<%=Fk_Module_Pic%>">
							<div style="padding:5px 0px 0px 10px;">
							<span class="sysBtn" id="UploadImage"><a id="chooseImgModule" style="cursor:pointer;display: inline-block;width: 100px;" class="Button"><b class="icon icon_add"></b>添加图片...</a></span>                                  
							</div>
							<label class="Normal" id="d_Thumbnail"></label>                              
    ...
    <tr id="Fk_Module_FileNames"<%If Instr(",1,2,5,6,7,",Fk_Module_Type)>0 Then%> style="display:none;"<%End If%>>
        <td align="right">生成页面：</td>
        <td><input name="Fk_Module_FileName" type="text" class="Input" id="Fk_Module_FileName" value="<%=Fk_Module_FileName%>" size="40" />
            *一旦确立不可修改</td>
    </tr>
    <tr id="Fk_Module_Urls"<%If Instr(",0,1,2,4,6,7,",Fk_Module_Type)>0 Then%> style="display:none;"<%End If%>>
        <td align="right">栏目转向链接：</td>
...
<div id="BoxBottom" style="width:93%; text-align:left; margin:0 auto;" class="tcbtm">
        ...
        <input type="hidden" name="MenuId" value="<%=MenuId%>" />
        <input type="submit" style="margin-left:113px;" onclick="Sends('ModuleEdit','Module.asp?Type=5',0,'',0,1,'MainRight','Module.asp?Type=1&MenuId=<%=MenuId%>');" class="Button" name="button" id="button" value="修 改" />
        <input type="button" onclick="layer.closeAll();$('select').show();" class="Button close" name="button" id="button" value="关 闭" />
</div>
</form>
<%
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
        <td style="border-bottom:0;"><textarea name="Fk_Module_Content" class="<%=bianjiqi%>" style="width:100%;" rows="15" id="Fk_Module_Content"><%=Fk_Module_Content%></textarea></td>
    </tr>
</table>
</div>
<div id="BoxBottom" style="width:93%;text-align:center; margin:0 auto;" class="tcbtm">
		<input type="hidden" name="Id" value="<%=Id%>" />
        <input type="hidden" name="MenuId" value="<%=MenuId%>" />
...
<div class="gnsztopbtn cdlmsx">
	<h3><%=Fk_Menu_Name%>菜单栏目排序</h3>
    <a class="back_lmlb" style="width:96px" href="javascript:void(0);" onclick="SetRContent('MainRight','Module.asp?Type=1&MenuId=<%=MenuId%>');return false;">返回栏目列表</a>
</div>
...
%>
        <tr>
            <td colspan="4" style="text-align:center; padding:5px 0; line-height:normal;">
            <input type="hidden" name="MenuId" value="<%=MenuId%>" />
...
Public Function ShowModuleListM(MenuIds,LevelId,TitleBack)
	...
	%>
	<tr>
		<td class="no1"><%=Rs2("Fk_Module_Id")%></td>
		<td align="left">&nbsp;&nbsp;<%=TitleBack%><%=Rs2("Fk_Module_Name")%></td>
		<td align="center"><span class="gnszxianshi <%if Rs2("Fk_Module_show")=0 then response.write "hidden"%>"></span></td>
		<td align="center"><%=FKFun.CheckModule(Rs2("Fk_Module_Type"))%></td>
		<td align="center" class="no5"><a title="预览 " target="_blank" href='<%If Rs2("Fk_Module_Type")=5 Then%>
		<%=Rs2("Fk_Module_Url")%><%else%><%=FKTemplate.GetGoUrl(Rs2("Fk_Module_Type"),Rs2("Fk_Module_Id"),Rs2("Fk_Module_Dir"),Rs2("Fk_Module_FileName"))%><%End If%>'>&nbsp;</a></td>
		<!--td align="center" style="display:none"><%=Temp%></td-->
		<td align="left" class="no6"><div class="gnszcaozuo" style="text-align:right; padding-right:20px;"><%if Rs2("Fk_Module_Type")=1 or Rs2("Fk_Module_Type")=2 then%><a title="栏目介绍 " href="javascript:void(0);" onclick="ShowBox('Module.asp?Type=9&MenuId=<%=MenuIds%>&Id=<%=Rs2("Fk_Module_Id")%>','栏目介绍','1000px','350px');" class="no1">&nbsp;</a><%end if%><a title="修改栏目设置 " href="javascript:void(0);" onclick="ShowBox('Module.asp?Type=4&MenuId=<%=MenuIds%>&Id=<%=Rs2("Fk_Module_Id")%>','修改栏目设置');" class="no2">&nbsp;</a><a title="增加子栏目 " href="javascript:void(0);" onclick="ShowBox('Module.asp?Type=2&MenuId=<%=MenuId%>&Mkid=<%=Rs2("Fk_Module_Id")%>&Lmlx=<%=Rs2("Fk_Module_Type")%>','增加子栏目');" class="no3">&nbsp;</a><a title="删除该栏目 " href="javascript:void(0);" onclick="DelIt('您确认要删除“<%=Rs2("Fk_Module_Name")%>”栏目？此操作不可恢复！','Module.asp?Type=6&Id=<%=Rs2("Fk_Module_Id")%>','MainRight','<%=Session("NowPage")%>');" class="no4">&nbsp;</a></div></td>
...
Public Function OrderModuleListM(MenuIds,LevelId,TitleBack)
	...
	<tr>
		...
		<td align="center"><input class="lmsx" name="Fk_Module_Order<%=Rs2("Fk_Module_Id")%>" size="5" type="text" id="Fk_Module_Order<%=Rs2("Fk_Module_Id")%>" value="<%=Rs2("Fk_Module_Order")%>" /></td>
	</tr>
<%
		If LevelId=0 Then
			TitleBacks="&nbsp;&nbsp;<span class='lm1'>└&nbsp;"
