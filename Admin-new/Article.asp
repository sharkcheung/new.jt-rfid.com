...
Select Case Types
	...
	Case 9
		Call ArticleOrderDo() '执行新闻栏目内容批量排序
	Case 10
		Call ArticleListRelated() '内容列表
	Case Else
...
<div id="ListContent">
	<div class="gnsztopbtn">
    	<h3><%=Fk_Module_Name%>栏目</h3><input name="SearchStr" value="<%=SearchStr%>" type="text" class="Input" id="SearchStr" style="vertical-align:middle;"/><input type="button" class="Button" onclick="SetRContent('MainRight','Article.asp?Type=1&ModuleId=<%=Fk_Module_Id%>&SearchStr='+escape(document.all.SearchStr.value));" name="S" Id="S" value="  查询  "  style="vertical-align:middle;"/>
        <h3>请选择栏目：</h3><select name="D1" id="D1" onChange="window.execScript(this.options[this.selectedIndex].value);" style="vertical-align:middle;">
              <option value="alert('请选择栏目');">请选择栏目</option>
        <%
        Call ModuleSelectUrl(Fk_Module_Menu,0,Fk_Module_Id)
        %>
        </select>
        <a class="tjia" href="javascript:void(0);" onclick="ShowBox('Article.asp?Type=2&ModuleId=<%=Fk_Module_Id%>','添加','1000px','500px');">添加</a>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <th align="left" class="ListTdTop" width="88">显示</th>
            <th align="center" class="ListTdTop" width="59">点击量</th>
            <th align="center" class="ListTdTop" width="59">排序</th>
            <th align="center" class="ListTdTop">转播微博</th>
            <th align="center" class="ListTdTop" width="150">添加时间</th>
            <th align="center" class="ListTdTop" width="119">操作</th>
        </tr>
<%
...
%>
        <tr>
            <td height="20" align="left" style="padding-left:33px"><input type="hidden" value="<%=Rs("Fk_Article_Id")%>" name="id[]"><input type="checkbox" name="ListId" class="Checks" value="<%=Rs("Fk_Article_Id")%>" id="List<%=Rs("Fk_Article_Id")%>" /></td>
            <td align="left" class="td1"><span style="line-height:22px; padding-right:15px; display:block"><%=Rs("Fk_Article_Title")%><%If Rs("Fk_Article_Color")<>"" Then%><span style="color:<%=Rs("Fk_Article_Color")%>">■</span><%End If%><%If Rs("Fk_Article_Url")<>"" Then%>[转向链接]<%End If%></span></td>
            <td align="center"  style="display:none"><%=Rs("Fk_Article_FileName")%></td>
            <td align="left"><%If Rs("Fk_Article_Show")=1 Then%><span class="gnszxianshi"></span><%Else%><span class="gnszxianshi hidden"></span><%End If%><%If Rs("Fk_Article_Pic")<>"" Then%><span style="color:#F00">[图]</span><%End If%><span style="display:none;">[模]</span><%If InStr(Fk_Article_Recommend,"推荐")>0 Then%><span title="<%=Replace(Fk_Article_Recommend,",","")%> ">[推]</span><%End If%><%If trim(Rs("Fk_Article_Ip")&" ")="1" Then%><span title="置顶 "><font color=blue style="font-weight:bolder;">[顶]</font></span><%End If%><%If Fk_Article_Subject<>"" Then%><span title="<%=Fk_Article_Subject%> ">[专]</span><%End If%></td>
            <td align="center"><%=Rs("Fk_Article_Click")%></td>
            <td height="20" align="center"><input type="text" value="<%=Rs("Px")%>" name="Fk_Article_px[]" style="width:25px;text-align:center"/></td>
            <td align="center">
			<a href="http://v.t.qq.com/share/share.php?url=<%=zfurl%>&appkey=3016f499d2714531819b4c5e5ec10cc1&site=&title=<%=server.URLEncode(Rs("Fk_Article_Title")&"("&Rs("Fk_Article_Keyword")&")")%>&pic=<%if left(Rs("Fk_Article_Pic"),4)<>"http" then
	...
	response.write Rs("Fk_Article_Pic")
            ...
            <td align="left" class="no6">
            <a class="no2" title="修改 " href="javascript:void(0);" onclick="ShowBox('Article.asp?Type=4&Id=<%=Rs("Fk_Article_Id")%>&ModuleId=<%=Fk_Module_Id%>','修改','1000px','500px');"></a>
            <a class="no4" title="删除 " href="javascript:void(0);" onclick="DelIt('您确认要删除“<%=Rs("Fk_Article_Title")%>”，此操作不可逆！','Article.asp?Type=6&Id=<%=Rs("Fk_Article_Id")%>','MainRight','<%=Session("NowPage")%>');"></a></td>
        </tr>
<%
			Rs.MoveNext
			i=i+1
...
%>
        <tr>
            <td height="30" colspan="8"  style="padding-left:33px;" class="NowPage">
            ...
            <input type="submit" value="删 除" class="Button" onClick="DelIt('确定要删除选中的内容吗？','Article.asp?Type=7&ListId='+GetCheckbox(),'MainRight','<%=Session("NowPage")%>');" style="vertical-align:middle;"> 
			<input type="submit" value="更新排序" class="Button" onClick="Sends('DelList','Article.asp?Type=9',0,'',0,1,'MainRight','<%=Session("NowPage")%>');" style="vertical-align:middle;"> 
...
'==========================================
'函 数 名：ArticleListRelated()
'作    用：内容列表
'参    数：
'==========================================
Sub ArticleListRelated()
	Session("NowPage")=FkFun.GetNowUrl()
	Dim SearchStr
	SearchStr=FkFun.HTMLEncode(Trim(Request.QueryString("SearchStr")))
	'判断权限
...
<div id="ListContent">
	<div class="gnsztopbtn">
    	<input name="SearchStr" value="<%=SearchStr%>" type="text" class="Input" id="SearchStr" style="vertical-align:middle;"/><input type="button" class="Button" onclick="SetRContent('MainRight','Article.asp?Type=1&ModuleId=<%=Fk_Module_Id%>&SearchStr='+escape(document.all.SearchStr.value));" name="S" Id="S" value="  查询  "  style="vertical-align:middle;"/>
...
<%
	Dim Rs2,ArticleUrl,zfurl
	Set Rs2=Server.Createobject("Adodb.RecordSet")
	Sqlstr="Select * From [Fk_Article] Where Fk_Article_Title Like '%%视频%%'"
	If SearchStr<>"" Then
		Sqlstr=Sqlstr&" And Fk_Article_Title Like '%%"&SearchStr&"%%'"
	End If
	Sqlstr=Sqlstr&" Order By Fk_Article_Ip desc,Px desc,Fk_Article_Time Desc"
...
%>
        <tr>
            <td height="20" align="left" style="padding-left:33px"><input type="hidden" value="<%=Rs("Fk_Article_Id")%>" name="id[]"><input type="checkbox" name="ListId" class="Checks" <%if inArray(Rs("Fk_Article_Id"), arrId) then%> checked <% end if%> value="<%=Rs("Fk_Article_Id")%>" id="List<%=Rs("Fk_Article_Id")%>" /><input type="hidden" value="<%=Rs("Fk_Article_Title")%>" name="title[]"></td>
            <td align="left" class="td1"><span style="line-height:22px; padding-right:15px; display:block"><%=Rs("Fk_Article_Title")%></span></td>
            <td align="center"><%=Rs("Fk_Article_Click")%></td>
            <td align="center"><%=Rs("Fk_Article_Time")%></td>
        </tr>
<%
...
%>
        <tr>
            <td height="30" colspan="4"  style="padding-left:33px;" class="NowPage">
            <input name="chkall" type="checkbox" id="chkall" value="select" onClick="CheckAll(this.form)" style="vertical-align:middle;">&nbsp;&nbsp;<label for="chkall" style="vertical-align:middle;">全选</label>&nbsp;&nbsp;&nbsp;&nbsp;
            
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%Call FKFun.ShowPageCodeRelated("Article.asp?Type=10&selectedIds="&Server.URLEncode(selectedIds)&"&SearchStr="&Server.URLEncode(SearchStr)&"&Page=",PageNow,PageAll,PageSizes,PageCounts)%></td>
        </tr>
<%
	Else
%>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
        ...
        <td align="right" colspan="2" width="100">排序：</td>
        <td><input style="width:60px" name="Fk_Article_px" type="text" id="Fk_Article_px" class="Input" size="6" maxlength="6" value="0" style="vertical-align:middle;"> （仅限数字，越大越排前）</td>
      </tr>
	<tr>
    ...
    <tr>
        ...
        <td>
		<input style="width:60px" name="Fk_Article_FileName" type="text" class="Input" id="Fk_Article_FileName" value="" size="30"  style="vertical-align:middle;"/>		&nbsp;*不可修改</td>
      </tr>
    <tr>
        ...
        <td colspan="5"><input  style="width:60px" name="Fk_Article_click" type="text" id="Fk_Article_click" class="Input" size="6" maxlength="6" value="<%=rnd_num%>" style="vertical-align:middle;"></td>
    </tr>
   <tr>
       ...
       <tr>
        ...
        <td colspan="4" style="padding:10px 0 10px;"><textarea name="Fk_Article_Content" class="<%=bianjiqi%>" style="width:100%;" rows="15" id="Fk_Article_Content"></textarea></td>
    </tr>
<%
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
        ...
        <td>点击量：</td>
        <td><input style="width:60px" name="Fk_Article_click" type="text" id="Fk_Article_click" class="Input" size="6" maxlength="6" value="<%=Fk_Article_Click%>" style="vertical-align:middle;">&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="Fk_Article_Time" id="Fk_Article_Time" value="1"  style="vertical-align:middle;"/><label for="Fk_Article_Time" style="vertical-align:middle;">&nbsp;更新时间</label></td>
    </tr>
	<tr>
		<td height="28" align="right">SEO标题：</td>
    ...
    <tr>
        ...
        <td><input style="width:60px" name="Fk_Article_px" type="text" id="Fk_Article_px" class="Input" size="4" maxlength="6" value="<%if isnull(Fk_Article_px) then response.write "0" else response.write Fk_Article_px%>" style="vertical-align:middle;">  (限数字,越大越排前)</td>
    </tr>
    <tr>
        <td height="25" align="right">SEO描述：</td>
		...
		<td><input name="Fk_Article_FileName" type="text" class="Input" id="Fk_Article_FileName" size="30" value="<%=Fk_Article_FileName%>"  style="vertical-align:middle; width:60px;"/>*不可修改</td>
    </tr>
     <tr>
        ...
        <td colspan="3"  style="padding:10px 0 10px;"><textarea name="Fk_Article_Content" class="<%=bianjiqi%>" style="width:100%;" rows="15" id="Fk_Article_Content"><%=Fk_Article_Content%></textarea></td>
    </tr>
<%
	Sqlstr="Select * From [Fk_Field] Where Fk_Field_Type=0 Order By Fk_Field_Id Asc"
	Rs.Open Sqlstr,Conn,1,1
...
'==============================
'函 数 名：ArticleOrderDo
'作    用：执行内容排序
'参    数：
'==============================
Sub ArticleOrderDo()
	Id=Replace(Trim(Request.Form("id[]"))," ","")
	Fk_Article_px=Replace(Trim(Request.Form("Fk_Article_px[]"))," ","")
	dim idarr
	If Id="" Then
	...
	comma=""
	if instr(Id,",")>0 and instr(Fk_Article_px,",")>0 then
		arrId = split(Id,",")
		arrPx = split(Fk_Article_px,",")
		if ubound(arrId)=ubound(arrPx) then
			for i = 0 to ubound(arrId)
				Call FKFun.ShowNum(arrPx(i),"排序序号必须是有效数字！")
				Sqlstr="Select Px,Fk_Article_Title From [Fk_Article] Where Fk_Article_Id="&arrId(i)&""
				Rs.Open Sqlstr,Conn,1,3
				if not Rs.eof then
					Application.Lock()
						Fk_Article_Title=Fk_Article_Title & comma&"【"&arrId(i)&"】"
						comma = ","
						Rs("Px")=arrPx(i)
						Rs.Update()
					Application.UnLock()
	...
	elseif instr(Id,",")=0 and instr(Fk_Article_px,",")=0 and Id>0 then
		Call FKFun.ShowNum(Fk_Article_px,"排序序号必须是有效数字！")
		Sqlstr="Select Px,Fk_Article_Title From [Fk_Article] Where Fk_Article_Id="&Id&""
		Rs.Open Sqlstr,Conn,1,3
		if not Rs.eof then
			Application.Lock()
				Fk_Article_Title="【"&Id&"】"
	...
	on error resume next
	dim log_content,log_ip,log_user
	log_content="批量更新ID为"&Fk_Article_Title&"的内容排序"
	log_user=Request.Cookies("FkAdminName")
