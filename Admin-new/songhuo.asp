...
<div id="ListContent">
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="shfssz">
          <tr >
            <th width="30%" align="center" >送货方式</th>
            <th width="10%" align="center" >排 序</th>
            <th width="15%" align="center" > 费 用</th>
            <th width="27%" align="center" > KEY</th>
            <th width="18%" align="center" >操 作</th>
          </tr>
          <%'dim i,j
		set rs=server.CreateObject("adodb.recordset")
		...
		j=j+1%>
          <tr>
            <form name="form1"  method="post">
              <td  align="center"><input style=" width:100px;" class="Input" name="subject" type="text" id="subject<%=j%>" size="20" value=<%=trim(rs("SongName"))%>>
              </td>
		...
		ondrop		= "return regInput(this,	/^[0-9]*$/,		event.dataTransfer.getData('Text'))">
              ...
              <td  STYLE='PADDING-LEFT: 20px'>
              	<input style="vertical-align:middle;" type="button" onclick="DelIt('','songhuo.asp?Type=2&id=<%=rs("SongID")%>&subject='+escape($('#subject<%=j%>').val())+'&songidorder='+$('#songidorder<%=j%>').val()+'&SongFei='+$('#SongFei<%=j%>').val()+'&key='+$('#key<%=j%>').val(),'MainRight','<%=Session("NowPage")%>');" name="Submit" class="Button" value="修 改">
                &nbsp;&nbsp;<input style="vertical-align:middle;"  type="button" class="Button" onclick="DelIt('您确认要删除，此操作不可逆！','songhuo.asp?Type=4&id=<%=rs("SongID")%>','MainRight','<%=Session("NowPage")%>');return false;" href="javascript:void(0);"  value="删 除"></input></td>
            </form>
          </tr>
          <%rs.movenext
			loop
			...
			set rs=nothing%>
          <tr>
            <th  align="center" > &nbsp; &nbsp; &nbsp; 添加送货方式</th><th colspan="4"></th>
          </tr>
          <tr>
            <form name="form2" method="post">
              <td  align="center"><input style="width:100px" class="Input" name="subject_add" type="text" id="subject_add" size="20">
              </td>
