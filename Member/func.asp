...
function get_count(id,intKey)
   ...
   select case intKey
	  ...
	  set get_rs=connn.execute(get_sql)
	  if get_rs.eof then
	     get_count=0
	  else
...
'网上支付方式
sub pay_type()
   set payrs=connn.execute("select * from Iheeo_Pay")
   if payrs.eof then
...
'   response.end
   set payrs=connn.execute(pay_sql)
   if payrs.eof then
...
function count_num(table,id)
   ...
   set countrs=connn.execute("select count(*) from "&table&""&where&"")
   count_num=countrs(0)
   countrs.close
   set countrs=nothing
end function
...
function p_price(pid,filed)
   ...
   set p_rs=connn.execute("select "&filed&" from Fk_Product where Fk_Product_Id="&pid&"")
   if p_rs.eof then
      p_price="商品已经删除或不存在!"
   else
      p_price=p_rs(0)
...
function M_memberID(M_name)
   set p_rs=connn.execute("select id from u_members where m_uid='"&M_name&"'")
   if p_rs.eof then
      M_memberID="参数错误!"
   else
      M_memberID=p_rs(0)
...
function M_fee(num,SongKey)
   set p_rs=connn.execute("select * from Iheeo_Delivery where SongKey="&SongKey&"")
   if p_rs.eof then
      M_fee=0
   else
      M_fee=p_rs(num)
...
sub product_list(maxperpage,titlen,ftitlen,imgwidth,imgheight)
   page=int(request("page"))
			  ...
			  if page>1 then
			     ...
			     set goodrs=connn.execute(goodsql)
				 response.Write "<ul>"
				 if goodrs.eof then
