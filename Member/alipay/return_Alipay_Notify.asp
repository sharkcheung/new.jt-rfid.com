...
If mysign=Request("sign") and ResponseTxt="true"   Then 	
	  conn.execute("update jh_pay set pro_paystatu=1 where pro_id='"&out_trade_no&"'")
	  conn.close
	  set conn=nothing
	response.write "<script language=javascript>alert('支付成功!请联系卖家发货!');window.location.href='../profile.asp'</script>"      '这里可以指定你需要显示的内容
	response.end
Else
	response.write "<script language=javascript>alert('支付失败!请在用户中心重新支付!');window.location.href='../profile.asp'</script>"
