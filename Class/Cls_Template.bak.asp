...
Class Cls_Template
	...
	Public Function InfoChange(TemplateCode)
		...
		If Not Rs.Eof Then
			...
			If Temp<>"" Then
				...
				dim intKk
				intKk=0
				Sqlstr="Select Fk_Word_Name,Fk_Word_Url From [Fk_Word] Order By Fk_Word_Id Desc"
				Rs.Open Sqlstr,Conn,1,1
				do while not Rs.Eof
					if instr(Temp,Rs("Fk_Word_Name"))>0 then
						intKk=intKk+1
						if intKk>3 then
							exit do
						end if
						Temp=Replace(Temp,Rs("Fk_Word_Name"),"<a href="""&Rs("Fk_Word_Url")&""" target=""_blank"" title="""&Rs("Fk_Word_Name")&""">"&Rs("Fk_Word_Name")&"</a>",1,1)
					end if
					Rs.MoveNext
				loop
			End If
	...
	Public Function ArticleListChange(TemplateCode)
		...
		If Not Rs.Eof Then
			...
			TemplateCode=Replace(TemplateCode,"{$ModuleContent$}",Rs("Fk_Module_Content"))
			Else
	...
	function AddInnerLink(byval content)
		dim Matches,objRegExp,strs,i,Match
		strs=content
		...
		Rs.Open Sqlstr,Conn,1,1
		do while not Rs.Eof
			if instr(strs,Rs("Fk_Word_Name"))>0 then
				'response.write Rs("Fk_Word_Name")&"<br/>"
				intKk=intKk+1
				if intKk>3 then	'排除过滤掉的关键词
					exit do
				end if
 				strs=replace(strs,Rs("Fk_Word_Name"),"<a href="""&Rs("Fk_Word_Url")&""" target=""_blank"" title="""&Rs("Fk_Word_Name")&""">"&Rs("Fk_Word_Name")&"</a>",1,1,1)
			end if
			Rs.MoveNext
		Loop
		rs.close 
	...
	Public Function GetGoUrl(ModuleType,ModuleId,ModuleDir,ModuleFileName)
		...
		ElseIf SiteHtml=1 Then
			Select Case ModuleType
				...
				Case 1
					GetGoUrl=SiteDir&"html/Article"&ModuleId&"/"
				Case 2
					GetGoUrl=SiteDir&"html/Product"&ModuleId&"/"
				Case 3
				...
				Case 7
					GetGoUrl=SiteDir&"html/Down"&ModuleId&"/"
				Case Else
					GetGoUrl="#"
			End Select
			If (ModuleType=1 Or ModuleType=2 Or ModuleType=7) And ModuleDir<>"" Then
					GetGoUrl=SiteDir&"html/"&ModuleDir&"/"
			End If
			If (ModuleType=0 Or ModuleType=3 Or ModuleType=4) And ModuleFileName<>"" Then
